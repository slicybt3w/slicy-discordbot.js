async function confirmation(message, validReactions, time = 60000) {
    ...
    if (!validReactions || validReactions.length !== 2)
        throw new ReferenceError(
            "slicy-discordbot => Invalid form body [validReactions]"
        );
    if (typeof time !== "number")
        throw new SyntaxError('slicy-discordbot => typeof "time" must be a number');
    if (!message.guild.me.hasPermission("MANAGE_MESSAGES"))
        return console.log(
            `error : Discord Client has to have "MANAGE_MESSAGES" permission.`
        );
