async function fetchTranscript(message, numberOfMessages) {
    ...
    if(!numberOfMessages) throw new ReferenceError('slicy-discordbot => "numberOfMessages" is not defined')
    if(typeof numberOfMessages !== "number") throw new SyntaxError('reconlx => typeof "numberOfMessages" must be a number')
    if(numberOfMessages >= 100) throw new RangeError('slicy-discordbot => "numberOfMessages" must be under 100 messages')
