...
async function EmbedPages(message, pages, pageTravel = false, emoji = ['⏪', '⏩'], time = 60000){
    ...
    if(!pages || typeof pages !== 'object') throw new SyntaxError('slicy-discordbot => Invalid form body [pages]')
    if(!emoji || emoji.length !== 2) throw new SyntaxError('slicy-discordbot => Invalid form body [emoji]')
    if(!time) throw new ReferenceError('slicy-discordbot => "time" is not defined')
    if(typeof time !== "number") throw new ReferenceError('slicy-discordbot => typeof "time" must be a number')
